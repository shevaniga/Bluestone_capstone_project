...
WITH nav_1yr AS (
    SELECT
        amfi_code,
        MAX(CASE
                WHEN nav_date = (SELECT MAX(nav_date) FROM nav_history)
                THEN nav
            END) AS nav_today,
        MAX(CASE
                WHEN nav_date <= DATE((SELECT MAX(nav_date) FROM nav_history), '-1 year')
                THEN nav
            END) AS nav_1yr_ago
    FROM nav_history
    GROUP BY amfi_code
...
LEFT JOIN scheme_details sd
    ON sd.amfi_code = fm.amfi_code
LEFT JOIN expense_ratio er
    ON er.amfi_code = fm.amfi_code
...
WITH returns AS (
    SELECT
        ...
        (
            ...
            /
            MAX(CASE
                    WHEN nav_date <= DATE((SELECT MAX(nav_date) FROM nav_history), '-3 year')
                    THEN nav
                END)
            - 1
        ) * 100 AS return_3yr_pct
    FROM nav_history
    GROUP BY amfi_code
)
SELECT
...
JOIN fund_master fm
    ON fm.amfi_code = ph.amfi_code
WHERE ph.report_month = (
